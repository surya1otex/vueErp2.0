<template>
  <div class="centered-container" id="app">
    
    <md-content class="md-elevation-3">
      <div class="title">
        <p style="color:red">{{ errors }}</p>
        <div class="md-title">ERP System</div>
        <div class="md-body-1"></div>
      </div>
      <form @submit.prevent="login_auth">
      <div class="form">
        
        <md-field>
          <label>E-mail</label>
          <md-input v-model="login.email" autofocus value=""></md-input>
        </md-field>
      ...
      <div class="actions md-layout md-alignment-center-space-between">
        ...
        <md-button class="md-raised md-primary" @click="login_auth">Log in</md-button>
      </div>
      </form>
...
export default {
  ...
  created() {
      //console.log('called from created hook');
   if (localStorage.getItem("setAuthentication")) {
    this.$store.commit("setAuthentication", false);
    localStorage.setItem("setAuthentication", false);
   }
  },
  methods: {
    auth() {
    ...
    login_auth() {
      ...
      if(this.login.email == '' && this.login.password == '') {
        this.errors = 'Please Enter Email and Password to Start a Session';
         return false;
      }
      ...
      .then(response => {
          ...
          if(response.status != '401' && response.data.success.token) {
           // alert("Token logic is approved");
              this.$store.commit("setAuthentication", true);
              localStorage.setItem("setAuthentication", true);
